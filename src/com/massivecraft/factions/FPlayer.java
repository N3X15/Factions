...
//        SpoutFeatures.updateAppearances(this.getPlayer());
    ...
    public Account getAccount() {
        if (!Econ.shouldBeUsed())
            return null;
        return iConomy.getAccount(this.getId());
    }
...
//            SpoutFeatures.updateAppearances(this.getPlayer());
    ...
    /*
     * public boolean isInOthersTerritory() { Faction factionHere = Board.getFactionAt(new FLocation(this)); return factionHere != null && factionHere.isNormal() && factionHere != this.getFaction(); }
     */

    /*
     * public boolean isInAllyTerritory() { return Board.getFactionAt(new FLocation(this)).getRelationTo(this) == Rel.ALLY; }
     */

    /*
     * public boolean isInNeutralTerritory() { return Board.getFactionAt(new FLocation(this)).getRelationTo(this) == Rel.NEUTRAL; }
     */
...
//        }
    ...
    public void leave(boolean makePay) {
        ...
        if (fplayers.size() == 1 && fplayers.get(0) == this) {
            // Transfer all money
            if (Econ.shouldBeUsed())
                Econ.transferMoney(this, myFaction, this, myFaction.getAccount().getHoldings().balance());
        }
    ...
    public boolean canClaimForFactionAtLocation(Faction forFaction,
        ...
        } else if (Conf.claimsMustBeConnected && !this.hasAdminMode() && myFaction.getLandRoundedInWorld(flocation.getWorldName()) > 0 && !Board.isConnectedLocation(flocation, myFaction) && (!Conf.claimsCanBeUnconnectedIfOwnedByOtherFaction || !currentFaction.isNormal())) {
